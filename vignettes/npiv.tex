...
\Plainauthor{Xiaohong Chen, Timothy Christensen, Sid Kankanala, Jeffrey
S. Racine}
\Plaintitle{\pkg{npiv}: Estimation and Inference for Nonparametric
Instrumental Variables Models Using R}
\Shorttitle{\pkg{npiv}: Estimation and Inference for Nonparametric IV
Models}
...
structural function and its derivatives. The package also allows for
constructing uniform confidence bands for the structural function and
its derivatives based on the methods proposed in \citet{CCQE}. All
methods apply to nonparametric regression as a special case.
}
...
\Address{
  ...
  URL: \url{https://economics.yale.edu/people/faculty/xiaohong-chen}\\~\\
    ...
    Yale University\\
    Department of Economics\\
  E-mail: \email{timothy.christensen@yale.edu}\\
  URL: \url{https://economics.yale.edu/people/timothy-christensen}\\~\\
      Sid Kankanala\\
    University of Chicago\\
    Booth School of Business\\
  E-mail: \email{sid.kankanala@chicagobooth.edu}\\
  URL: \url{https://www.chicagobooth.edu/faculty/directory/k/sid-kankanala}\\~\\
      Jeffrey S. Racine\\
    McMaster University\\
...
\citep{causalML}. In this paper, we introduce the \proglang{R} \citep{R}
package \pkg{npiv} \citep{NPIV}. This package implements several
...
newly accessible to \proglang{R} users. The package \pkg{npiv} is
...
The main methods implemented in \pkg{npiv} are (i) a data-driven choice
...
Additional functionality of the \pkg{npiv} package allows for
...
\citet{ChenPouzo}. All methods in \pkg{npiv} subsume nonparametric
...
implemented for the first time in the \proglang{R} package \pkg{npiv}.
There do not exist implementations with a similar scope in any other
software languages, either. The \proglang{R} packages \pkg{np}
\citep{NPPkg} and \pkg{crs} \citep{CRS} have functionality for
estimation of \(h_0\) and its derivative in NPIV models. However, these
packages implement different estimators than \pkg{npiv}, namely kernel
and regression spline methods, respectively, using Tikhonov and
Landweber--Fridman regularization. Further, \pkg{npiv} is the first to
...
functionality of the package \pkg{npiv} and describes its
...
package. The package \pkg{npiv} is available from CRAN at
\url{https://CRAN.R-project.org/package=npiv}.
...
The package \pkg{npiv} makes available a set of novel, fully-data driven
...
widely used package \pkg{ivreg} \citep{IVREG} for linear IV models.
