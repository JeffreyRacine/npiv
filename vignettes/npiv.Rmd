...
We being by briefly describing sieve NPIV estimators. Consider approximating the unknown structural function $h_0$ using a linear combination of $J$ basis functions, denoted $\psi_{J1},\ldots,\psi_{JJ}$:
...
is the projection matrix onto the instrument space,\footnote{We let $A^-$ denote the generalized (or Moore--Penrose) inverse of a matrix $A$.} and $\mathbf Y = (Y_1,\ldots,Y_n)^\top$ is a $n \times 1$ vector.
...
\]
with $\hat{\mathbf u}_J^* = (\hat u_{1,J}\varpi_1,\ldots,\hat u_{n,J}\varpi_n)^\top$ denoting a multiplier bootstrap version of $\hat{\mathbf u}_J$, where $(\varpi_i)_{i=1}^n$ are IID standard normal $N(0,1)$ draws independent of the data.
...
@CCK establish uniform asymptotic coverage guarantees for these UCBs for both NPIV and nonparametric regression models. They also show that the UCBs contract at the optimal sup-norm rate (up to logarithmic factors).
...
* `formula`: a symbolic formula specifying the model to be estimated. The syntax is the same as the package \pkg{ivreg} [@IVREG]. For exampe, `y ~ x1 + x2 | x1 + z1 + z2` where `y` is the dependent variable, `x1` is an exogenous regressor, `x2` is an endogenous regressor, and `z1` and `z2` are instrumental variables.
...
will perform nonparameric regression of `y` on `x1` and `x2`, with a data-driven choice of sieve dimension. Note that this choice of sieve dimension will be rate-optimal for sup-norm. This choice may therefore differ from the choice of sieve dimension selected by other methods that target mean-square error (i.e., $L^2$ norm), such as cross validation, because the optimal choice for sup-norm and $L^2$ norm diverge at different rates. As before, data-driven UCBs for the conditional mean function and its derivative will also be computed by default. 

Estimators based on a deterministic choice of sieve dimension and undersmoothed UCBs can be computed by passing `J.x.segments`. Note that `K.w.segments` and `K.w.smooth` are both redundant for nonparametric regression.
...
We illustrate the usage of `npiv` with an application to nonparametric Engel curve estimation [@BCK]. Engel curves explain how consumers' budget shares on different categories of goods and services vary as a function of total household expenditure. Understanding these substitution patterns across expenditure categories is important for policy analysis and modeling demand.

We use data from the 1995 British Family Expenditure Survey, following @BCK, @CCQE, and several other works. We focus on the subset of 1655 married or cohabitating couples with the head of household employed and aged between 20 and 55 and with at most two children. The data are available as part of \pkg{npiv} in the data frame `Engel95`.
...
We wish to estimate the Engel curve $h_0$ for household budget share on a particular expenditure category (e.g., food). The model is
\[
 Y_i = h_0(X_i) + u_i
\]
where $Y_i$ is budget share for household $i$ on the expenditure category of interest and $X_i$ is log total household expenditure. As is well known [@BBL; @BBC], log total household expenditure is endogenous:
\[
 \E[Y_i | X_i] \neq h_0(X_i),
\]
which follows from the fact that total expenditure is the sum of expenditure across categories.
We therefore follow @BBL, @BCK, and several other works and use log household income as and instrument for expenditure.

We first estimate the Engel curve for food using log income as an instrument. We foucs on the subset of households with 1-2 children.
...
As `J.x.segments` and `K.w.segments` are not passed in this call (and therefore revert to their default `NULL` values), `npiv()` uses a data-driven choice of sieve dimension and constructs data-driven uniform confidence bands using the methods from @CCK. The data-driven choice of sieve dimension are $J = 4$ and $K = 8$ (the sum of segments and degree for each basis). As $X$ and $W$ are both scalar and the degree of each basis is the default values of 3 for a cubic B-spline basis for $X$ and 4 for a quartic B-spline basis for $W$. The run time represents the total time to perform data-driven choice of sieve dimension and to construct the data-driven UCBs for $h_0$ and its derivative.

Alternatively, we can estimate $h_0$ and its derivative using a deterministic choice of $J$ and $K$ and construct undersmoothed UCBs based on the methods of @CCQE by passing both `J.x.segments` and `K.w.segments`. As the justification for the UCBs is undersmoothing, we must choose a $J$ and $K$ larger than the optimal value so that bias is of smaller order than sampling uncertainty. We choose to estimate with `J.x.segments = 2` and `K.w.segments = 5`, which corresponds to $J = 5$ and $K = 9$.

```{r}
fm.food.det <- npiv(food ~ logexp | logwages, J.x.segments = 2, K.w.segments = 5)
```

```{r food-dd, dev = 'pdf', echo = FALSE, fig.show = 'hide'}
...
```{r, food-det, echo = FALSE, dev = 'pdf', fig.show = 'hide'}
...
The estimated Engel curves for food and corresponding UCBs are plotted below. Both the data-driven and undersmoothed estimates looks similar and are downward-sloping. This seems reasonable from an economic standpoint: food is a necessary good, and so it should decrease as a proportion of overall household expenditure as household expenditure increases. Note, however, that the undersmoothed estimates and UCBs are wigglier than their data-driven counterparts, as they use a higher-dimensional basis to approximate $h_0$.
...
```{r food-dd-deriv, dev = 'pdf', echo = FALSE, fig.show = 'hide'}
...
```{r, food-det-deriv, echo = FALSE, dev = 'pdf', fig.show = 'hide'}
...
These differences are also apparent when we compare the estimated derivatives of the Engel curves. Note also from the UCBs for derivatives that while the estimated derivatives are generally negative, the zero function lies almost entirely inside both sets of UCBs. Therefore, it is difficult to conclude that the Engel curves for food are significantly downwards-sloping. 
...
```{r fuel-dd, dev = 'pdf', echo = FALSE, fig.show = 'hide'}
## Plot the estimated Engel curve and data-driven UCBs
plot(logexp, fuel,
...
```{r fuel-dd-deriv, dev = 'pdf', echo = FALSE, fig.show = 'hide'}
## Plot the estimated Engel curve derivative and data-driven UCBs
...
We see from the estimated Engel curve and its derivative that budget shares on fuel decrease significantly at low income levels and then flatten out completely at moderate to high income levels.
