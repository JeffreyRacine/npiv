...
npivJ <- function(Y,
    ...
    Z.sup.boot <- matrix(NA,B,NROW(J1.J2))
    ...
    for(ii in 1:NROW(J1.J2)) {
      ...
      ## Mar 11 - Tim indicates that segments for W are a deterministic
      ## function of J... so is there 1 B.w for J1 and 1 for J2?
      ## Awaiting Tim's response
      ...
      for(b in 1:B) Z.sup.boot[b,ii] <- max((Psi.x.J1.eval%*%tmp.J1%*%(U.J1*rnorm(length(Y)))-
